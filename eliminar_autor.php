...
<form method="post" action="">
        <h2>eliminar Autores</h2>
        <input type="number" name="id_autor" placeholder="ID Autores" required>
        <input type="submit" value="Consultar Autores" name="consultar_administrador">
    </form>
    ...
    if (isset($_POST['consultar_administrador'])) {
        ...
        if (mysqli_num_rows($resultados) > 0) {
            ...
            <?php endif; ?>
                <input type="submit" value="Eliminar Autores" name="eliminar_administrador">
                <input type="button" value="Cancelar" onclick="window.location.href='eliminar_administrador.php'">
              
            </form>
            <?php
        } else {
            echo "No se encontro Autores .";
        }
    }
    ...
    if (isset($_POST['modificar_administrador'])) {
       ...
       if (isset($_FILES['Imagen']) && $_FILES['Imagen']['error'] == UPLOAD_ERR_OK) {
        ...
        $consulta1 = "UPDATE Autores SET nombre_autor='$nombre_autor', nacionalidad='$nacionalidad', fecha_nacimiento='$fecha_nacimiento', genero_literario='$genero_literario', id_editorial='$id_editorial', premios='$premios', imagen='$imagen'  WHERE id_autor = '$id_autor'";
     ...
     if (isset($_POST['eliminar_administrador'])) {
        ...
        $consulta_administrador = "DELETE FROM Autores WHERE id_autor = '$id_autor'";
        mysqli_query($conex, $consulta_administrador); 
        $consulta1 = "DELETE FROM Autores WHERE id_autor = '$id_autor'";

        if (mysqli_query($conex, $consulta1)) {
            echo "Autores eliminado correctamente.";
        } else {
